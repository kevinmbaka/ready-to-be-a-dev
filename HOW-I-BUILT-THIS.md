...
### 4.3 The three CI failures and their fixes (how the build went green)

The first three cloud builds failed. Each cause was found by reading the real Actions log
rather than guessing. The root theme: **Capacitor's Xcode target bundles files that are
`.gitignore`d** (they're normally generated by `cap copy`), so on a fresh CI checkout they
did not exist.

| # | Failing step | Real cause | Fix |
|---|--------------|-----------|-----|
| 1 | Install Capacitor deps & sync iOS | `npm ci` + `cap sync` combo failed on the runner | Split into separate steps; `npm install --legacy-peer-deps` |
| 2 | Copy web app into iOS project | `npx cap copy ios` failed on macOS, so `App/App/public` + `capacitor.config.json` were never created | Replaced `cap copy` with a plain `cp -R` + writing `capacitor.config.json` directly |
| 3 | Build UNSIGNED archive | `error: lstat(.../App/App/config.xml): No such file or directory` — the target also bundles `config.xml`, also git‑ignored | Generate `config.xml` in the same step |

Build #4 passed every step. The resulting `.ipa` was downloaded and verified:

- correct `Payload/App.app/` layout (40 entries)
- **64‑bit Mach‑O** ARM binary (`cffaedfe`) built by Xcode 16.4
- `CFBundleIdentifier=com.readydev.app`, display name "Ready to be a Dev",
  `MinimumOSVersion=15.0`, `UIDeviceFamily=[1,2]` (iPhone + iPad)
- unsigned (no `embedded.mobileprovision`) — exactly what Sideloadly expects
- the web app inside at `Payload/App.app/public/` (12 files)

### 4.4 Hosting the app files on the website

`builds/android/ReadyDev.apk` and `builds/ios/ReadyDev-unsigned.ipa` are committed, so
GitHub Pages serves them, and `index.html` has a downloads section linking to both. Details:

- `.nojekyll` at the repo root makes Pages serve every file verbatim.
- The downloads section is **hidden inside the packaged native apps** (`app.js` checks that
  `location.protocol` is http/https), because those relative paths don't exist in a bundle.
- The workflow re-publishes `builds/ios/ReadyDev-unsigned.ipa` after each successful build,
  so the hosted file never goes stale. That commit only touches `builds/ios/**`, which is
  **not** in the workflow's `paths:` trigger, so it cannot re-trigger itself.

### 4.5 What you still need for iOS
